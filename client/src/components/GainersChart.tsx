 ...
 * Sol: Top 10 kazanan | Sağ: Top 10 kaybeden
 */
...
function Chart({
  ...
  useEffect(() => {
    ...
    const rows = g.selectAll<SVGGElement, HeatmapStock>(".bar-row")
      ...
      .on("click", (_, d) => onSelect(d.code));

    // Bar
    rows.append("rect")
      .attr("x", 0)
      .attr("y", 4)
      .attr("width", (d) => xScale(Math.abs(d.rate)))
      .attr("height", BAR_H - 8)
      .attr("fill", color)
      .attr("rx", 3)
      .attr("opacity", 0.85)
    ...
    // % değer (barın sağında)
    rows.append("text")
      .attr("x", (d) => xScale(Math.abs(d.rate)) + 5)
      .attr("y", BAR_H / 2)
      .attr("dominant-baseline", "middle")
      .attr("font-size", "9px")
    ...
    <div style={{ display: "flex", gap: 24, padding: "16px 20px", flexWrap: "wrap" }}>
      ...
      <div>
        ...
        {gainers.length > 0 ? (
          ...
          <div style={{ color: "#6b7280", fontSize: "12px", fontFamily: "'Space Grotesk', sans-serif", padding: "12px 0" }}>
